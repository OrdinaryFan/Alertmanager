...
import (
	...
	"github.com/prometheus/alertmanager/provider/alertmysql"
...
func main() {
	...
	db, _ := sql.Open("mysql", "root:101314@tcp(localhost:3306)/alertmanager2?charset=utf8")
	if *hwaddr == "" {
	...
	*routePrefix = "/" + strings.Trim(*routePrefix, "/")
	...
	<-term
	store := make(chan int, 1)
	go func() {
		db.Query("TRUNCATE TABLE alertmanager2.labels")
		//fmt.Println(err)
		db.Query("TRUNCATE TABLE alertmanager2.annotations")
		//fmt.Println(err)
		db.Query("DELETE FROM alertmanager2.alerts")
		db.Query("ALTER TABLE alertmanager2.alerts AUTO_INCREMENT = 1")
		insert, _ := alerts.GetPendAlerts()
		alertmysql.Putmysql(db, insert)
		db.Query("TRUNCATE TABLE alertmanager2.silences")
		silences.Putmysql(db)
		db.Close()
		store <- 1
	}()
	<-store
