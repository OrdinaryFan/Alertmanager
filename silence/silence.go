...
//从数据库中添加数据到silence中
func (s *Silences) SetmysqlSilence(sil *pb.MeshSilence) error {

	if err := validateSilence(sil.Silence); err != nil {
		fmt.Println("Set wrong")
		return errors.Wrap(err, "silence invalid")
	}

	st := gossipData{sil.Silence.Id: sil}
...
func (s *Silences) Printsilences() {
	for _, a := range s.st {
		fmt.Println(a)
	}
}

func (s *Silences) Putmysql(db *sql.DB) {
	//将数据装入数据库中
	for _, a := range s.st {
		...
		checkErr(err)
		var tmpEndsAt time.Time
		if tmp, _ := a.Silence.EndsAt.Zone(); tmp == "UTC" {
			h, _ := time.ParseDuration("1h")
			tmpEndsAt = a.Silence.EndsAt.Add(8 * h)
		} else {
			tmpEndsAt = a.Silence.EndsAt
		}
		loc, _ := time.LoadLocation("Local")
		tmp1, _ := time.ParseInLocation("2006-01-02 15:04:05", a.Silence.StartsAt.String()[:19], loc)
		tmp2, _ := time.ParseInLocation("2006-01-02 15:04:05", tmpEndsAt.String()[:19], loc)
		tmp3, _ := time.ParseInLocation("2006-01-02 15:04:05", a.Silence.UpdatedAt.String()[:19], loc)
		tmp4, _ := time.ParseInLocation("2006-01-02 15:04:05", a.ExpiresAt.String()[:19], loc)
		_, err = stmt.Exec(a.Silence.Id, matchertmp, tmp1.String()[:19], tmp2.String()[:19], tmp3.String()[:19], commentstmp, a.Silence.CreatedBy, a.Silence.Comment, tmp4.String()[:19])
		checkErr(err)
	}
}

func (s *Silences) Getmysql(db *sql.DB) {
	//从数据库中读取数据
	...
	for rows.Next() {
		...
		if err != nil {
			fmt.Println(err)
			fmt.Println("Error in getting the silence from mysql")
		}
		json.Unmarshal([]byte(matchertmp), &matchers) //json解析到结构体里面
		json.Unmarshal([]byte(Comments), &comments)   //json解析到结构体里面
		loc, _ := time.LoadLocation("Local")
		tmp1, _ := time.ParseInLocation("2006-01-02 15:04:05", starsat, loc)
